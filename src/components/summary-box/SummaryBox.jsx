...
import { format, isAfter, lastDayOfMonth, parseISO, isBefore, isEqual, isSameDay } from 'date-fns'
...
export const SummaryBoxSpecial = ({ item, data }) => {
    ...
    const calcRevenue = (data) => {
        ...
        data.forEach((transaction) => {
            ...
            if (transaction.type === 'income') {
                if ((isAfter(new Date(date), new Date(firstDateOfMonth)) || isEqual(new Date(date), new Date(firstDateOfMonth))) && (isAfter(new Date(lastDateOfMonth), new Date(date)) || isEqual(new Date(lastDateOfMonth), new Date(date)))) {
    ...
    return (
        <Box custom centered fullheight relative test={true}>
            {x ? <div className="summary-box-special">
                <div className="summary-box-special__title">
                    {item.title}
                </div>
                <div className={`summary-box-special__value ${styles.fadeAnimation}`}>
                    {revenue !== 0 ? `RON ${revenue}` : <CircularProgress />}
                </div>
                <Player
                    src={images.income_dashboard_lottie
                    }
                    loop
                    autoplay
                />
            </div> :
                <Skeleton variant="rectangular" width={'100%'} height={'100%'} sx={{ position: 'absolute', borderRadius: '30px', backgroundColor: 'gray' }} />
            }
